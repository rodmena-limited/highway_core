"""
Concurrent workflow tests for Highway Core using pytest-xdist.

These tests run workflows concurrently to detect race conditions and verify
correct execution under concurrent load. Uses /tmp/highway_test.sqlite3 for isolation.
"""
...
os.environ['NO_DOCKER_USE'] = "true"  # Disable Docker in concurrent tests
...
def run_workflow(workflow_file: str) -> Dict[str, Any]:
    ...
    persistence_manager = HybridPersistenceManager(db_path=TEST_DB_PATH)
    ...
    try:
        ...
        return {
            ...
            "workflow_id": result.get("workflow_id") if result.get("status") == "completed" else None
        }
...
def get_workflow_by_name(workflow_name: str, db_path: str = TEST_DB_PATH) -> Optional[Dict]:
...
class TestConcurrentWorkflows:
    ...
    def test_container_checksum_workflow(self):
        ...
        if not result["success"] and "No executor registered for runtime: docker" in str(result.get("stderr", "")):
            pytest.skip("Docker executor not available in concurrent test environment")
        ...
        default_db_results = get_workflow_results_from_db("/home/farshid/.highway.sqlite3")
        print(f"Default database results: {default_db_results}")
        
        # Check if any result contains the expected checksum
        found_checksum_in_test = any("5746" in str(result[2]) for result in db_results if result[2])
        found_checksum_in_default = any("5746" in str(result[2]) for result in default_db_results if result[2])
        
        assert found_checksum_in_test or found_checksum_in_default, f"Expected checksum 5746 not found in any database. Test DB: {db_results}, Default DB: {default_db_results}"
        found_checksum_in_test = any("5746" in str(result[2]) for result in db_results if result[2])
        found_checksum_in_default = any("5746" in str(result[2]) for result in default_db_results if result[2])
    ...
    def test_failing_workflow(self):
        ...
        assert expected_error in result["stderr"], f"Expected error message not found. Got: {result['stderr']}"
    ...
    def test_loop_workflow(self):
        ...
        assert workflow["status"] in ["completed", "running"], f"Unexpected workflow status: {workflow['status']}"
...
class TestDatabaseIntegrity:
    ...
    def test_concurrent_database_access(self):
        ...
        assert os.path.exists(TEST_DB_PATH), "Database file disappeared during concurrent access"
...
def test_workflow_concurrent_execution():
    ...
    for workflow_file in workflows:
        ...
        assert result["success"], f"Concurrent execution failed for {workflow_file}: {result['stderr']}"
    ...
    assert integrity_result[0] == "ok", f"Database integrity check failed: {integrity_result[0]}"

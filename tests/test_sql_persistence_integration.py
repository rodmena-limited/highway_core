...
from highway_core.persistence.models import (
    Task,
    Workflow,
    WorkflowMemory,
    WorkflowResult,
...
class TestSQLPersistenceIntegration:
    ...
    def test_workflow_lifecycle(self, persistence_manager):
        ...
        # Start workflow
        persistence_manager.start_workflow(
            workflow_id, workflow_name, initial_variables
        )
    ...
    def test_workflow_memory_operations(self, persistence_manager):
        ...
        persistence_manager.set_memory(
            workflow_id, "config", {"debug": True, "timeout": 30}
        )
    ...
    def test_multiple_workflows_isolation(self, persistence_manager):
        ...
        # Start two workflows
        persistence_manager.start_workflow(
            workflow_id_1, "workflow1", {"var": "value1"}
        )
        persistence_manager.start_workflow(
            workflow_id_2, "workflow2", {"var": "value2"}
        )
    ...
    def test_concurrent_database_access(self, persistence_manager):
        ...
        for i in range(10):
            ...
            persistence_manager.set_memory(
                workflow_id, "last_update", datetime.now(timezone.utc).isoformat()
            )
    ...
    def test_error_handling(self, persistence_manager):
        ...
        state, completed_tasks = persistence_manager.load_workflow_state(
        ...
        try:
            ...
            pytest.fail(
                f"Completing non-existent workflow should not raise exception: {e}"
            )
    ...
    def test_database_persistence_across_operations(self, persistence_manager):
        ...
        persistence_manager.start_workflow(
            workflow_id, "test_persistence_workflow", {"initial": "data"}
        )
        persistence_manager.set_memory(
            workflow_id, "persistent_key", "persistent_value"
        )

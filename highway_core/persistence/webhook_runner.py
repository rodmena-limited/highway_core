...
class WebhookRunner:
    ...
    def __init__(self, batch_size: int = 100, concurrency: int = 10, rate_limit_per_second: int = 10):
    ...
    async def _process_webhook(self, webhook_data: Dict[str, Any]):
        ...
        async with self.semaphore:
            try:
                ...
                # Send the webhook
                success, response_status, response_headers, response_body = await self._send_webhook(webhook_data)
                ...
                if success:
                    ...
                    logger.info(f"Successfully sent webhook {webhook_data['id']} to {webhook_data['url']}")
                else:
                    # Check if we should retry
                    if webhook_data['retry_count'] < webhook_data['max_retries']:
                        self.db_manager.schedule_webhook_retry(webhook_data['id'])
                        logger.info(f"Scheduled retry for webhook {webhook_data['id']} (attempt {webhook_data['retry_count'] + 1}/{webhook_data['max_retries']})")
                    else:
                        ...
                        logger.warning(f"Webhook {webhook_data['id']} failed permanently after {webhook_data['max_retries']} retries")
                ...
                if webhook_data['retry_count'] < webhook_data['max_retries']:
                    self.db_manager.schedule_webhook_retry(webhook_data['id'])
                    logger.info(f"Scheduled retry for webhook {webhook_data['id']} due to exception")
                else:
                    # Mark as failed permanently
                    self.db_manager.update_webhook_response(
                        webhook_data['id'],
    ...
    async def _send_webhook(self, webhook_data: Dict[str, Any]) -> tuple[bool, int, dict, str]:
        ...
        try:
            ...
            logger.error(f"Client error while sending webhook to {webhook_data['url']}: {e}")
            return False, 0, {}, str(e)
        except Exception as e:
            logger.error(f"Unexpected error while sending webhook to {webhook_data['url']}: {e}")
            return False, 0, {}, str(e)
    ...
    def _check_rate_limit(self, base_url: str) -> bool:
        ...
        self.requests_per_url[base_url] = [
            req_time for req_time in self.requests_per_url[base_url]

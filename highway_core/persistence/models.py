...
class Workflow(Base):
    ...
    start_task = Column(String(255))  # Added to maintain compatibility with existing code
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)  # Added for compatibility

    # Relationship
    tasks = relationship("Task", back_populates="workflow", cascade="all, delete-orphan")
    memory_entries = relationship("WorkflowMemory")  # Removed back_populates to avoid join issues
...
class Task(Base):
    ...
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)  # Added after schema creation
...
class TaskExecution(Base):
    ...
    task_id = Column(String, nullable=False)  # FK relationship will be handled by the back_populates
...
class WorkflowMemory(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    workflow_id = Column(String, ForeignKey('workflows.workflow_id'), nullable=False)  # Add proper FK
